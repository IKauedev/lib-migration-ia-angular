...
export const PROVIDERS = {
    ...
    ollama: {
        name: "Ollama (local)",
        models: ["llama3", "llama3.1", "mistral", "codellama", "phi3", "gemma2", "qwen2.5-coder"],
...
export function loadConfig() {
    ...
    const envMappings = [
        ...
        ["openrouter", "OPENROUTER_API_KEY", { model: "anthropic/claude-opus-4-5" }],
    ];
    ...
    const activeOk = activeCfg ? .apiKey ||
        (PROVIDERS[config.activeProvider] ? .noKeyRequired && (activeCfg ? .endpoint || activeCfg ? .model));
